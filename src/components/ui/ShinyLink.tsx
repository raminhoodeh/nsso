...
export default function ShinyLink({ href, children, className = '', ...props }: ShinyLinkProps) {
    return (
        <div
            className={`p-[0.75px] rounded-[12px] group ${className}`}
            style={{
        ...
        >
            <a
                href={href}
                {...props}
                className="relative h-[42px] w-full rounded-[12px] flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed transition-all hover:scale-[1.02] active:scale-[0.98]"
                style={{
